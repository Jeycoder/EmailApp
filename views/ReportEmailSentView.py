...
class ReportEmailSentView:
    ...
    def __init__(self, controller, root):
        ...
        new_window.wm_title("Report Emails Sent")
        new_window.wait_visibility()

        self.email_frame = Frame(new_window)
        self.email_frame.pack()
        label = Label(new_window, text="Report Emails Sent", font="Calibri 14 bold")
        self.main_table()

        label.place(x=220, y=10)
        Label(new_window, text="Filter: ").place(x=50, y=50)

        self.txt_search = Entry(new_window, width=30, textvariable=self.text_to_search)
        self.txt_search.place(x=100, y=50)
        Button(new_window, width=10, text="Search",
               command=self.search).place(x=370, y=50)
    ...
    def data_table(self):
        try:
            ...
            for row in list_lists:
                data[index] = dict()
                data[index]['Id'] = row[0]
                data[index]['List'] = row[1]
                data[index]['Template'] = row[2]
                data[index]['Num. Emails'] = row[3]
                index += 1
